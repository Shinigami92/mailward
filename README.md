...
> It has been rebuilt as a **self-hostable app**: a Rust engine + GraphQL API and a Vue web UI. The
> engine, API and UI are functional today (classify / cleanup / inspect, dry-run by default); the
> remaining work is packaging it as a Docker image (see the [roadmap](#roadmap)). The v1 rule
...
> Docker image + Compose file land in a later phase. For now this is a developer setup - see
> **[CONTRIBUTING.md](CONTRIBUTING.md)** for prerequisites, the repo layout and the `just` task list.
...
Run them separately with `just dev-api` / `just dev-web` if you prefer two terminals.
...
- [ ] **Phase 5**: Docker image + Compose, published to GHCR ← _here_
- [ ] **Later**: IMAP IDLE / cron triggers, Prometheus/Grafana stats, optional local-LLM classifier
