...
section(class="space-y-3")
  div
    ...
    p(v-if="description" class="text-sm text-ui-fg-muted") {{ description }}
  ...
  textarea(v-model="content" spellcheck="false" class="h-96 w-full rounded-md border border-ui-line-strong bg-ui-surface p-3 font-mono text-xs leading-relaxed text-ui-fg shadow-inner focus:border-ui-fg-muted focus:outline-none")
  div(class="flex items-center gap-3")
    button(type="button" :disabled="fetching || status.kind === 'saving'" class="rounded-md bg-ui-accent px-4 py-2 text-sm font-medium text-ui-accent-fg hover:opacity-90 disabled:opacity-50" @click="save") Save
    span(v-if="status.kind === 'saved'" class="text-sm text-emerald-600 dark:text-emerald-400") Saved.
    span(v-if="status.kind === 'error'" class="text-sm text-rose-600 dark:text-rose-400") {{ status.message }}
</template>
